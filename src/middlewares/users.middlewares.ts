      ...
      username: {
        ...
        custom: {
          options: async (value, { req }) => {
            ...
            //tìm user bằng username3 người dùng muốn cập nhật
            const user = await databaseService.users.findOne({
              username: value
            })
            if (user) {
  ...
  checkSchema(
    {
      old_password: {
        ...
        custom: {
          options: async (value, { req }) => {
            //lấy decoded_authorization từ req
            const { user_id } = req.decoded_authorization as TokenPayLoad
            //tìm user trong database
            const user = await databaseService.users.findOne({
              _id: new ObjectId(user_id)
            })
            //nếu mà user ko tồn tại thì báo lỗi
            if (!user) {
              throw new ErrorWithStatus({
                message: USERS_MESSAGES.USER_NOT_FOUND,
                status: HTTP_STATUS.NOT_FOUND
            ...
            //nếu mà user tồn tại thì check xem old_password có đúng với password trong database hay không?
            const { password } = user
